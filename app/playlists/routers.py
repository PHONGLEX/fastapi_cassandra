...
def playlist_video_add_post_view(request: Request, db_id: uuid.UUID, is_htmx=Depends(is_htmx), title: str = Form(...), url: str = Form(...)):
    ...
    redirect_path = data.get('path') or "/playlists/{{db_id}}"
...
def playlist_remove_video_item_view(request: Request, db_id: uuid.UUID
                                    , host_id: str
